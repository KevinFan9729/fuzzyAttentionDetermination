...
def recalibrate():
  global recalibrate_first_time, past, x_center, x2_center, x_left, x2_left, x_right, x2_right, recal_flag

  t=threading.Timer(3, recalibrate)#after 3 second, run function (maybe try 6 seconds?)
  t.setDaemon(True)
  t.start()
  ...
  try:
    with mp_face_mesh.FaceMesh(
      ...
      while cap.isOpened():
        ...
        c_results = face_mesh.process(img)

        # Draw the face mesh annotations on the image.
        img.flags.writeable = True
        image_h,image_w=img.shape[:2]

        if c_results.multi_face_landmarks:
          for face_landmarks in c_results.multi_face_landmarks:
            normalized_xy=np.array([([p.x,p.y]) for p in face_landmarks.landmark])
            points_xy=np.multiply(normalized_xy,[image_w, image_h]).astype(np.float32)

            current = points_xy[FOREHEAD][0][0]
...
def fuzzyEye(iris_pos, iris2_pos, headDir):
  ...
  eyePos['med'] = fuzz.trimf(eyePos.universe, [x_fuzzy_left-10+int((x_fuzzy_center-x_fuzzy_left-10)/2), x_fuzzy_center, x_fuzzy_right+10-int((x_fuzzy_right+10-x_fuzzy_center)/2)])
  ...
  eyePos2['med'] = fuzz.trimf(eyePos2.universe, [x2_fuzzy_left-10+int((x2_fuzzy_center-x2_fuzzy_left-10)/2), x2_fuzzy_center, x2_fuzzy_right+10-int((x2_fuzzy_right+10-x2_fuzzy_center)/2)])
